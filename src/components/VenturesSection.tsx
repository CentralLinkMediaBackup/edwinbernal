...
const ventures: Venture[] = [
  ...
  {
    index: "03",
    name: "Prosperity Fire Protection",
  ...
  },
  {
    ...
    period: "Jan — Nov 2025",
    description:
      "Dallas-based digital solutions company helping small businesses grow their online presence with affordable, high-quality website design and digital branding services.",
    stats: [{ value: 100, prefix: "$", suffix: "K", label: "Revenue in 8 Months" }],
  },
];
          ...
          {ventures.map((venture, i) => (
            <Reveal key={venture.index} delay={i * 0.05}>
              <article className="row-hover rule group grid grid-cols-1 lg:grid-cols-12 gap-x-8 gap-y-6 py-10 md:py-14 px-2 md:px-4 -mx-2 md:-mx-4">
                {/* Index + period */}
                <div className="lg:col-span-2 flex lg:flex-col justify-between gap-2">
                  <span className="font-display text-sm text-muted-foreground">({venture.index})</span>
                  <span className="eyebrow text-muted-foreground">{venture.period}</span>
                </div>
